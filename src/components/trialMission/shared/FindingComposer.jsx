      ...
      <div className="flex items-center justify-between">
        ...
        {!showFindingForm && (
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 px-3.5 py-1.5 text-xs font-bold text-white shadow-sm shadow-blue-500/20 hover:from-blue-700 hover:to-indigo-700 transition-all duration-200"
          >
      ...
      {showFindingForm && (
        ...
        >
          <div className="flex items-center justify-between border-b border-blue-100 pb-2">
            ...
            <span className="text-[11px] text-blue-600 font-medium">Step 1 of Investigation</span>
          </div>
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2 text-xs text-slate-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none"
            />
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs text-slate-900 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none"
              >
      ...
      {workspaceLoading ? (
        ...
        <div className="space-y-3">
          {findings.map((f, idx) => (
            <div
              key={f.id || idx}
              className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4 space-y-1.5 transition hover:bg-slate-100/70"
            >
              <div className="flex items-start justify-between gap-2">
                <h4 className="text-xs font-bold text-[#0b1a36]">{f.statement}</h4>
                <span className="rounded-full bg-blue-100 border border-blue-200 px-2 py-0.5 font-mono text-[10px] font-bold text-blue-800">
                  Finding #{idx + 1}
                </span>
              </div>
