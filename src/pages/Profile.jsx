...
export default function Profile({ profile, onRestart, onSave }) {
  ...
  if (!profile) {
    return (
      <section className="min-h-screen bg-gradient-to-br from-[#f4f8fd] via-[#edf3fb] to-[#dfeaf7] px-6 py-10 text-slate-800 font-sans">
  ...
  const inputClass =
    "w-full rounded-xl border border-[#D3E3F5] bg-white px-4 py-3 text-sm text-slate-800 outline-none transition focus:border-slate-400 placeholder:text-slate-400 shadow-2xs";

  const labelClass = "text-xs font-bold text-slate-600 uppercase tracking-wider";

  return (
    <section className="min-h-screen bg-gradient-to-br from-[#f4f8fd] via-[#edf3fb] to-[#dfeaf7] px-6 py-10 text-slate-800 text-left font-sans">
      <form onSubmit={handleSubmit} className="mx-auto max-w-5xl space-y-10">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-[#D3E3F5] pb-6">
          <div className="flex items-center gap-4">
            ...
            {success && !authLoading && (
              <span className="text-xs text-emerald-800 font-semibold bg-emerald-50 border border-emerald-300 px-2.5 py-1 rounded-full">
                ✓ {success}
              </span>
            )}
          </div>
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-xl bg-[#0b1a36] hover:bg-[#122b59] px-5 py-2.5 text-xs font-bold text-white transition disabled:opacity-60 shadow-xs"
          >
        ...
        <div className="flex justify-start">
          <div className="relative group cursor-pointer">
            <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-[#D3E3F5] shadow-xs flex items-center justify-center bg-gradient-to-br from-cyan-500 to-blue-600">
              {avatarUrl ? (
                <img
                  src={avatarUrl}
                  alt="Profile Avatar"
            ...
            <div className="absolute bottom-0 right-0 bg-[#0b1a36] text-white p-1.5 rounded-full border-2 border-white shadow-xs">
              <Camera size={11} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <FormField label="Phone Number" className={labelClass}>
              <div className="flex rounded-xl border border-[#D3E3F5] overflow-hidden focus-within:border-slate-400 bg-white shadow-2xs">
                <select
        ...
        <div className="border-t border-[#D3E3F5] pt-8 space-y-4">
          ...
          <div className="flex items-start gap-3 rounded-xl bg-white p-4 border border-[#D3E3F5] text-xs text-slate-600 shadow-2xs">
          ...
          <button
            type="button"
            onClick={() => logout()}
            className="rounded-xl border border-slate-300 text-slate-700 hover:bg-slate-50 hover:text-slate-900 px-5 py-2.5 text-xs font-bold transition bg-white shadow-2xs flex items-center gap-1.5"
          >
            <LogOut size={13} />
            <span>Log out</span>
          </button>
...
function FormField({ label, children, className }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className={className || "text-xs font-bold text-slate-600 uppercase tracking-wider"}>
        {label}
      </label>
      {children}
    </div>
