...
import { ArrowLeft, RefreshCw, ChevronLeft, ChevronRight, TrendingUp, DollarSign, Activity, ShieldCheck, Loader2, Target, CheckCircle2, AlertCircle, ArrowRight, Lock, Trophy, Star, Sparkles } from "lucide-react";
...
const DASHBOARD_MODULES = [
  {
    ...
    description:
      "Discover your core traits and how they align with various work environments.",
    status: "Not Started",
    active: true,
  },
  {
    ...
    description:
      "Explore the activities and industries that truly excite and motivate you.",
    status: "Locked",
    active: false,
  },
  {
    ...
    description:
      "Identify your natural strengths and areas where your talents shine brightest.",
    status: "Locked",
    active: false,
  },
  {
    ...
    title: "Work Values",
    description:
      "Pinpoint the workplace culture and values that lead to your long-term fulfillment.",
    status: "Locked",
    active: false,
  },
];

const DASHBOARD_CAREERS = [
  {
    tag: "Data Science",
    title: "Market Analyst",
    image:
      "url('https://images.unsplash.com/photo-1556155092-8707de31f9c6?auto=format&fit=crop&w=1200&q=80')",
  },
  {
    tag: "Creative Arts",
    ...
    <>
      <section className="min-h-screen w-full overflow-hidden bg-[radial-gradient(circle_at_top_left,_#dff7ff_0%,_#edf3ff_34%,_#dce7ff_68%,_#f6fbff_100%)] px-4 py-6 sm:px-8 sm:py-9">
        <div className="mx-auto w-full max-w-6xl overflow-hidden rounded-[28px] border border-[#bdd4ff] bg-white/88 shadow-[0_24px_70px_rgba(34,74,150,0.18)] backdrop-blur">
            ...
            <div className="mt-8">
              <div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]">
                <motion.div
                  ...
                  className="rounded-[18px] border border-[#e2e8f0] bg-white p-8 shadow-[0_22px_60px_rgba(15,35,80,0.08)]"
                >
                  <div className="flex flex-col items-center gap-8 text-center">
                    <div className="relative h-56 w-56 rounded-full border-8 border-slate-100 bg-white shadow-[0_16px_34px_rgba(15,35,80,0.08)] sm:h-60 sm:w-60">
                      <div className="absolute inset-0 rounded-full border-8 border-slate-100 border-t-blue-600" />
                      <div className="absolute inset-7 flex flex-col items-center justify-center rounded-full bg-white text-center">
                        <span className="text-5xl font-semibold tracking-[-0.04em] text-slate-950">0%</span>
                        <span className="mt-2 text-sm uppercase tracking-[0.28em] text-slate-500">Complete</span>
                      </div>
                    </div>

                    <div className="space-y-3">
                      <h2 className="text-2xl font-semibold text-slate-950">The Adventure Awaits</h2>
                      <p className="max-w-xl text-sm leading-6 text-slate-500 sm:text-base">
                        You haven't started your assessment yet. Take the first step today!
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="inline-flex items-center justify-center gap-2 rounded-full bg-blue-600 px-7 py-3 text-sm font-semibold text-white shadow-[0_16px_40px_rgba(59,130,246,0.24)] transition duration-200 hover:-translate-y-0.5 hover:bg-blue-700"
                    >
                      Start Assessment
                      <ArrowRight size={18} />
                    </button>
                ...
                <motion.div
                  ...
                  className="space-y-5"
                >
                  <div className="flex items-center justify-between rounded-[18px] border border-[#e2e8f0] bg-white p-5 shadow-[0_18px_40px_rgba(15,35,80,0.06)]">
                    <h2 className="text-lg font-semibold text-slate-950">Discovery Steps</h2>
                    <div className="rounded-full bg-blue-50 px-3 py-1 text-sm font-semibold text-blue-700">4 Modules</div>
                  </div>
                  ...
                  <div className="space-y-4">
                    {DASHBOARD_MODULES.map((module) => (
                      <div
                        key={module.id}
                        className={`flex items-start gap-4 rounded-[16px] border px-5 py-5 transition ${
                          module.active
                            ? "border-blue-100 bg-white shadow-[0_16px_32px_rgba(59,130,246,0.12)] hover:border-blue-200"
                            : "border-slate-200 bg-slate-100 text-slate-500"
                        }`}
                      >
                        <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-full ${
                          module.active ? "bg-blue-600 text-white" : "bg-slate-200 text-slate-500"
                        }`}>
                          {module.active ? "P" : <Lock size={18} />}
                        </div>

                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            <h3 className={`text-base font-semibold ${module.active ? "text-slate-950" : "text-slate-500"}`}>
                              {module.title}
                            </h3>
                            <span className={`rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] ${
                              module.active ? "bg-slate-100 text-slate-700" : "bg-slate-200 text-slate-500"
                            }`}>
                              {module.status}
                            </span>
                          </div>
              ...
              <div className="mt-10 space-y-4">
                <div>
                  <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-500">Future Possibilities</p>
                  <h2 className="mt-3 text-3xl font-semibold text-slate-950 sm:text-4xl">What your DNA could reveal...</h2>
                </div>

                <div className="grid gap-5 lg:grid-cols-3">
                  {DASHBOARD_CAREERS.map((card) => (
                    <motion.div
                      key={card.title}
                      whileHover={{ scale: 1.02 }}
                      className="relative overflow-hidden rounded-[20px] bg-cover bg-center text-white shadow-[0_22px_70px_rgba(15,35,80,0.16)]"
                      style={{ backgroundImage: card.image }}
                    >
                      <div className="absolute inset-0 bg-gradient-to-t from-slate-950/90 via-slate-950/30 to-transparent" />
                      <div className="absolute inset-x-0 bottom-0 p-5">
                        <span className="inline-flex rounded-full bg-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-white/90">
                          {card.tag}
                        </span>
          ...
          {!showDashboard && !isLoadingCareers && !loadError && !result ? (
            <div className="mt-8">
              {/* Progress Section */}
